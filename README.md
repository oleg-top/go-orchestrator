...
```
Также sqlite - база данных, которую я использую в данном проекте - почему-то требует установленный gcc. Если у вас он не установлен, то с запуском также могут возникнуть проблемы. Для решения нужно просто установить компилятор [gcc](https://code.visualstudio.com/docs/cpp/config-mingw) из интернета.
